...
def profile(username):
    if username:
        user= Users.query.filter_by(username=username).first()
        current_user_= user
        all_post = Posts.query.filter_by(user_id=current_user_.id).all()
        if not user:
            return "no username found"
    current_user_= current_user
    
    # quering for all posts withe user that will be generated by the cureent user in the route
    all_post = Posts.query.filter_by(user_id=current_user_.id).all()


    return render_template("profile.html",current_user=current_user_, all_post=all_post)
